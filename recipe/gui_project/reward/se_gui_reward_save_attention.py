...
SAVED_ATTENTION_DIR = Path("/home/kaiyu/Project/gui-project/gui-verl/dataset/temp/saved_attention")
...
def maybe_save_val_attention(extra_info: dict) -> None:
    split = str(extra_info.get("split", "")).lower()
    if split != "val":
        return

    image_attention_by_layer = extra_info.get("image_attention_by_layer")
    if image_attention_by_layer is None or len(image_attention_by_layer) == 0:
        return

    index = str(extra_info.get("index", "")).strip()
    if not index:
        return

    SAVED_ATTENTION_DIR.mkdir(parents=True, exist_ok=True)
    attention_array = np.asarray(image_attention_by_layer)
    np.save(SAVED_ATTENTION_DIR / f"{index}.npy", attention_array)
...
def compute_score(
    ...
    maybe_save_val_attention(extra_info)
